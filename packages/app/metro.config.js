...
const ignorePackages = othersWorkspace.map((f) => new RegExp(`${path.resolve(packages, f)}/.*`)); // Ignore workspace packages
...
module.exports = {
  ...
  resolver: {
    blacklistRE: blacklist(
      [new RegExp(`^${escape(path.join(root, 'node_modules', 'react-native'))}\\/.*$`)].concat(
        othersWorkspace.map(
